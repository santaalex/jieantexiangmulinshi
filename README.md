...
2. `engineering-product-definition-fuinno-windows-x64-0.3.1.zip`
3. `giant-tube-engineering-fuinno-windows-x64-0.4.1.zip`
...
0.3.1 与 0.4.1 是针对首轮 Windows HIL 的增量修复。已经 PASS 的 CAD 0.7.0 外形结果和缓存证据直接复用，不再重跑耗时的 STEP 几何；只复测 OCR 私有运行时、PDF 证据提取、Giant 安全启动，以及前置闸门满足后的后续链路。失败时最多做一次诊断性重试，不扩充测试样件。
